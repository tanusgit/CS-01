...
public class DateProfile {
		...
		char searchGender;
		//int romance-an int from 1 to 10, indicating the importance of romance to the applicant.
		int romance = 0;
		...
		/*
		 * double fitValue(DateProfile partner), which returns a number from 0.0 
		 * (very bad fit) to 1.0 (perfect fit).  The public instance method compares
		 *  the calling object (this) to the object passed as a parameter.  
		 *  This method should call three private methods determineGenderFit( ... ), 
		 *  determineRomanceFit( ... ) and determineFinanceFit( ... ), that will be used 
		 *  to return intermediate results for each of the three factors.  It should compute
		 *   the final fit value that it returns by returning a 0 if gender is not 
		 *   compatible (regaredless of the other values) and return the average of 
		 ...
		 * object and the passed parameter object.  You have to compare gender compatibility 
		 * completely: i.e., there must be mutual consent
		 * on this one!  It is used by the public fitValue(), not directly by the client main().
		 */
		 ...
		 * returns a number from (but not including) 0.0 to (and including) 1.0 depending 
		 * on the romance compatibility of the calling object and the passed parameter 
		 * object. The romance numbers should be highest (1.0) if the two values are equal 
		 * (both 3, both 5, both 7) and lowest (perhaps a small non-zero value like .1)  
		 * if their difference is 9. It is used by the public fitValue(), not directly
		 *  by the client main().
		 */
		 ...
		 * returns a number  from (but not including) 0.0 to (and including) 1.0 depending 
		 * on the finance compatibility of the calling object and the passed parameter 
		 * object. The finance numbers should be highest (1.0) if the two values are equal
		 *  (both 3, both 5, both 7) and lowest (perhaps a small non-zero value like .1) 
		 *   if their difference is 9. It is used by the public fitValue(), not directly
		 *    by the client main().
		 */
